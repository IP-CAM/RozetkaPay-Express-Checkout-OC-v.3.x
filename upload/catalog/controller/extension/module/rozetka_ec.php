...
class ControllerExtensionModuleRozetkaEc extends Controller {			
	...
	public function callback() {				
		...
		if(!$result) {
			...
			if(!empty($result['is_success']) && $result['is_success']) {
				...
				$order_status_id = $this->config->get('module_rozetka_ec_order_fail_status_id');
			}
	...
	public function getFooterJs() {
		if($this->config->get('module_rozetka_ec_status')) {
			$data['button_cart'] = $this->config->get('module_rozetka_ec_button_cart');
			$data['button_cart_js'] = html_entity_decode($this->config->get('module_rozetka_ec_button_cart_js'), ENT_QUOTES, 'UTF-8');
			$data['position_button_cart_js'] = $this->config->get('module_rozetka_ec_position_button_cart_js');
			
			$data['button_product'] = $this->config->get('module_rozetka_ec_button_product');
			$data['button_product_js'] = html_entity_decode($this->config->get('module_rozetka_ec_button_product_js'), ENT_QUOTES, 'UTF-8');
			$data['position_button_product_js'] = $this->config->get('module_rozetka_ec_position_button_product_js');
			
			$data['button_checkout'] = $this->config->get('module_rozetka_ec_button_checkout');
			$data['button_checkout_js'] = html_entity_decode($this->config->get('module_rozetka_ec_button_checkout_js'), ENT_QUOTES, 'UTF-8');
			$data['position_button_checkout_js'] = $this->config->get('module_rozetka_ec_position_button_checkout_js');
